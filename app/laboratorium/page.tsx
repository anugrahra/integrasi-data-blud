...
export default function LaboratoriumPublicPage() {
	...
	const [showMetodeInfo, setShowMetodeInfo] = useState(false);
	...
	useEffect(() => {
		const fetchSupabaseData = async () => {
			...
			if (error) {
				console.error(
					"Gagal narik data dari Supabase:",
	...
	const availableYears = useMemo(() => {
		const years = Array.from(
			new Set(labData.map((d) => d.tgl_uji.substring(0, 4))),
		);
	...
	const availableMonthsInYear = useMemo(() => {
		...
		const months = Array.from(
			new Set(dataYear.map((d) => d.tgl_uji.substring(5, 7))),
		);
	...
	const currentPeriodPrefix =
		selectedMonth === "ALL" ? selectedYear : `${selectedYear}-${selectedMonth}`;
	...
	const prevPeriodPrefix = useMemo(() => {
		...
		const allExistingMonths = Array.from(
			new Set(labData.map((d) => d.tgl_uji.substring(0, 7))),
		).sort();
	...
	const periodLabel = useMemo(() => {
		...
		return selectedMonth === "ALL"
			? `TAHUN ${selectedYear}`
			: format(parseISO(`${selectedYear}-${selectedMonth}-01`), "MMMM yyyy", {
					locale: id,
				}).toUpperCase();
	...
	const prevPeriodLabel = useMemo(() => {
		...
		return selectedMonth === "ALL"
			...
			: format(parseISO(`${prevPeriodPrefix}-01`), "MMMM yyyy", {
					locale: id,
				}).toUpperCase();
	}, [selectedMonth, prevPeriodPrefix]);

	const currentParamConfig = useMemo(
		() => PARAMETERS.find((p) => p.id === activeParam) || PARAMETERS[0],
		[activeParam],
	);

	// --- STATISTIK UMUM (Berdasarkan Prefix Aktif) ---
	const stats = useMemo(() => {
		const dataPeriod = labData.filter((d) =>
			d.tgl_uji.startsWith(currentPeriodPrefix),
		);
		...
		dataPeriod.forEach((row) => {
			PARAMETERS.forEach((p) => {
				...
				if (val !== null && val !== undefined && val !== "") {
					...
					// SINKRONISASI LOGIKA BAKU MUTU:
					// Gunakan fungsi isTidakMemenuhi eksisting yang sudah pintar membaca row & modifier
					const tidakAman = isTidakMemenuhi(p.id, val, row);

					if (!tidakAman) totalParameterAman++;
				}
			});
		});

		const persentase =
			totalParameterDiuji > 0
	...
	const getDelta = (key: string, invertedLogic = false) => {
		const dataCurr = labData.filter((d) =>
			d.tgl_uji.startsWith(currentPeriodPrefix),
		);

		// Amankan agar tidak memfilter semua data jika prevPeriodPrefix kosong
		const dataPrev = prevPeriodPrefix
			? labData.filter((d) => d.tgl_uji.startsWith(prevPeriodPrefix))
			: [];

		// Fungsi pembantu interceptor lokal untuk data mikrobiologi bernilai <1
		const getInterceptedValue = (row: any) => {
			let val = Number(row[key] || 0);
			if (
				(key === "coliform" || key === "ecoli") &&
				val === 1 &&
				row[`${key}_modifier`] === "<"
			) {
				return 0;
		...
		const avgCurr = dataCurr.length
			? dataCurr.reduce((a, c) => a + getInterceptedValue(c), 0) /
				dataCurr.length
			: 0;
		const avgPrev = dataPrev.length
			? dataPrev.reduce((a, c) => a + getInterceptedValue(c), 0) /
				dataPrev.length
			: 0;
	...
	const kpiSlides = useMemo(() => {
		...
		return [
			{
				title: "Tingkat Kejernihan",
				value: turbidity.avg,
				unit: "NTU",
				icon: Activity,
				desc: "Batas Maks. 3 NTU",
				diff: turbidity,
				color: "text-blue-600",
				bg: "bg-blue-50",
			},
			{
				title: "Bebas Kuman & Bakteri",
				value: ecoli.avg,
				unit: "CFU",
				icon: Bug,
				desc: "Harus 0 CFU",
			...
			{
				title: "Keseimbangan Air (pH)",
				value: ph.avg,
				unit: "",
				icon: FlaskConical,
				desc: "Ideal di angka 6.5 - 8.5",
				diff: ph,
				color: "text-purple-600",
				bg: "bg-purple-50",
			},
			{
				title: "Warna & Estetika",
				value: color.avg,
				unit: "TCU",
				icon: Droplets,
				desc: "Air Jernih Tidak Berwarna",
				diff: color,
				color: "text-teal-600",
				bg: "bg-teal-50",
			},
	...
	useEffect(() => {
		const timer = setInterval(
			() => setActiveSlide((p) => (p + 1) % kpiSlides.length),
			6000,
		);
		...
		return PIPELINE_ORDER.map((lokasi) => {
			...
			const getAvg = (matches: any[]) => {
				...
				const sum = matches.reduce(
					(acc, curr) => acc + Number(curr[activeParam] || 0),
					0,
				);
			...
			const isBreach =
				currentValue !== null && currentParamConfig.max !== undefined
	...
	const handleGenerateAiSummary = async () => {
		...
		try {
			...
			console.error(e);
			setAiSummary(
				"Koneksi ke otak AI terputus. Pastikan API Key valid dan server menyala.",
			);
		} finally {
	...
	const filteredData = useMemo(() => {
		...
		if (searchTerm) {
			...
			data = data.filter(
				(d) =>
					d.lokasi.toLowerCase().includes(lower) ||
	...
	const tableAverages = useMemo(() => {
		...
		// Fungsi sum global menggunakan metode DL/2 untuk SEMUA parameter
		const sum = (key: string) =>
			filteredData.reduce((acc, row) => {
				let val = Number(row[key] || 0);

				// ATURAN GLOBAL: Jika kolom memiliki modifier '<',
				// nilainya otomatis disubstitusi menjadi setengah dari nilai limit deteksi tersebut
				if (row[`${key}_modifier`] === "<") {
					val = val / 2;
				}
		...
		const avgColiform = sum("coliform") / len;
		const avgEcoli = sum("ecoli") / len;
		...
		return {
			...
			coliform:
				avgColiform === 0
					? "0"
					: avgColiform % 1 === 0
						? avgColiform.toFixed(0)
						: avgColiform.toFixed(2),
			ecoli:
				avgEcoli === 0
					? "0"
					: avgEcoli % 1 === 0
	...
	const handleExportExcel = () => {
		...
		if (tableAverages) {
			wsData.push([
				...
				tableAverages.ph,
				tableAverages.suhu,
				tableAverages.klorin,
				tableAverages.kekeruhan,
				tableAverages.tds,
				"-",
				tableAverages.warna,
				tableAverages.fe,
				tableAverages.mn,
				tableAverages.flourida,
				tableAverages.nitrat,
				tableAverages.nitrit,
				tableAverages.coliform,
				tableAverages.ecoli,
				"-",
			]);
		}
		...
		autoTable(doc, {
			...
			head: [
				[
					{
						content: "LOKASI UJI",
						styles: { halign: "left", fillColor: [23, 23, 23] },
					},
					{
						content: "TANGGAL",
						styles: { halign: "center", fillColor: [23, 23, 23] },
					},
					{
						content: "pH",
						styles: { halign: "center", fillColor: [30, 58, 138] },
					},
					{
						content: "KEKERUHAN",
						styles: { halign: "center", fillColor: [30, 58, 138] },
			...
			body: filteredData.map((d) => [
				d.lokasi,
				format(parseISO(d.tgl_uji), "dd/MM/yy"),
				d.ph,
				d.kekeruhan,
				d.tds,
				d.fe,
			...
			foot: tableAverages
				? [
						[
							...
							{
								content: tableAverages.kekeruhan,
								styles: {
									halign: "center",
									fillColor: [23, 23, 23],
									textColor: [255, 255, 255],
	...
	// --- FUNGSI CEK BAKU MUTU DINAMIS (Menggunakan Hoisting) ---
	function isTidakMemenuhi(paramId: string, value: any, row?: any) {
		const param = PARAMETERS.find((p) => p.id === paramId);
		if (!param) return false;

		// Pastikan nilai diubah ke tipe Number agar komparasi matematika selalu akurat
		let val = Number(value);

		// LOGIKA KHUSUS MIKROBIOLOGI:
		// Jika objek row dikirimkan, periksa apakah nilainya 1 dengan modifier '<'
		if (row && (paramId === "coliform" || paramId === "ecoli")) {
			const modifier = row[`${paramId}_modifier`];
			if (val === 1 && modifier === "<") {
				val = 0; // Ubah ke 0 untuk pengecekan standar baku mutu saja
			}
		}

		// Jalankan pengecekan ambang batas baku mutu
		if (param.max !== undefined && val > param.max) return true;
		if (param.min !== undefined && val < param.min) return true;
								...
								>
									...
									{availableMonthsInYear.map((month) => (
										<option key={month} value={month}>
											{format(parseISO(`2000-${month}-01`), "MMMM", {
												locale: id,
											}).toUpperCase()}
										</option>
									))}
				...
				<div className="max-w-[1400px] mx-auto px-6 mt-8 space-y-8">
					<div className="bg-blue-50 border border-blue-200 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-sm gap-4">
						<div className="flex items-center gap-4">
							...
							<div>
								<h2 className="text-xs font-bold text-blue-800/70 uppercase tracking-widest mb-0.5">
						...
						<div className="bg-emerald-600 rounded-xl shadow-sm relative overflow-hidden flex flex-col justify-center items-center p-8 text-white border border-emerald-500 group">
							...
							<div className="relative z-10 flex flex-col items-center w-full text-center">
								...
								<h2 className="text-6xl lg:text-7xl font-black tracking-tighter leading-none mb-1 drop-shadow-md">
									{stats.persentase}
									<span className="text-3xl text-emerald-200 ml-1">%</span>
								</h2>
							...
							<div className="flex-1 relative">
								{kpiSlides.map((slide, index) => {
									...
									return (
										<div
											key={index}
											className={`absolute inset-0 p-6 flex flex-col items-center justify-center text-center transition-all duration-500 transform ${index === activeSlide ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8 pointer-events-none"}`}
										>
											<div
												className={`w-14 h-14 rounded-full ${slide.bg} flex items-center justify-center mb-4`}
											>
											...
											<div className="flex items-baseline gap-1.5 mb-4">
												<p
													className={`text-5xl font-mono font-black tracking-tighter ${slide.color}`}
												>
													{slide.value}
												</p>
											...
											<div
												className={`px-4 py-2 rounded-lg border flex items-center gap-2 ${slide.diff.isGood ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-amber-50 border-amber-200 text-amber-700"}`}
											>
												{slide.diff.trend === "down" ? (
							...
							<div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-10">
								{kpiSlides.map((_, i) => (
									<div
										...
										className={`h-1.5 rounded-full cursor-pointer transition-all ${i === activeSlide ? "bg-blue-600 w-6" : "bg-neutral-200 w-2 hover:bg-neutral-300"}`}
									></div>
								))}
							</div>
						</div>
							...
							<div className="relative z-10 flex flex-col items-center">
								...
								{(() => {
									...
									if (score >= 98) {
										...
										borderCode =
											"border-emerald-200 shadow-[0_0_15px_rgba(16,185,129,0.4)]";
									...
									return (
										<>
											<div
												className={`w-28 h-28 rounded-full border-8 ${borderCode} flex items-center justify-center mb-4 bg-white transition-all duration-300`}
											>
												<span
													className={`text-6xl font-black ${colorCode} drop-shadow-sm`}
												>
													{grade}
												</span>
											</div>
							...
							<div className="flex flex-col md:flex-row justify-between md:items-end mb-5 gap-4">
								<div>
									<h3 className="text-lg font-bold flex items-center gap-2 uppercase tracking-tight text-neutral-900">
										<Map className="w-5 h-5 text-blue-600" /> Topografi
										Pengaliran (HULU KE HILIR)
									</h3>
									<p className="text-xs text-neutral-500 mt-1">
										Mengurutkan titik *sampling* berdasarkan jarak alami aliran
										air dari reservoir.
									</p>
								</div>
								...
								<label className="flex items-center gap-3 cursor-pointer shrink-0 bg-white px-4 py-2 rounded-lg border border-neutral-300 shadow-sm hover:border-blue-400 transition-colors">
									<div className="relative">
										...
										/>
										<div
											className={`block w-10 h-5 rounded-full transition-colors ${showComparison ? "bg-indigo-500" : "bg-neutral-300"}`}
										></div>
										<div
											className={`dot absolute left-1 top-1 bg-white w-3 h-3 rounded-full transition-transform ${showComparison ? "transform translate-x-5" : ""}`}
										></div>
									</div>
								...
								<div className="absolute top-6 left-12 flex gap-4 text-[10px] font-mono bg-neutral-50 px-3 py-2 rounded border border-neutral-200 z-10 opacity-90 hover:opacity-100 transition-opacity shadow-sm">
									<span className="flex items-center gap-1.5">
										<div
											...
											style={{ backgroundColor: currentParamConfig.color }}
										></div>
										Area Terpilih
									</span>
									{showComparison && (
										<span className="flex items-center gap-1.5 ml-2">
											<div className="w-4 h-0.5 border-t-2 border-dashed border-neutral-400"></div>
											Data Historis
										</span>
									)}
									<span className="flex items-center gap-1.5 ml-2">
										<div className="w-4 h-0.5 border-t-2 border-solid border-red-500"></div>
										Batas Maks/Min
									</span>
								</div>
									...
									>
											...
											domain={[
												(dataMin: number) =>
													currentParamConfig.min !== undefined
														? Number(
																(
																	Math.min(dataMin, currentParamConfig.min) *
																	0.8
																).toFixed(2),
															)
														: 0,
												(dataMax: number) => {
													const maxVal =
														currentParamConfig.max !== undefined
															? Math.max(dataMax, currentParamConfig.max)
															: dataMax;
										...
										<defs>
											...
											>
												<stop
													offset="5%"
													stopColor={currentParamConfig.color}
													stopOpacity={0.6}
												/>
												<stop
													offset="95%"
										...
										{currentParamConfig.max !== undefined && (
											<ReferenceLine
												...
												label={{
													position: "top",
													value: `MAKS: ${currentParamConfig.max} ${currentParamConfig.unit}`,
													fill: "#ef4444",
													fontSize: 10,
													fontWeight: "900",
													fontFamily: "monospace",
										...
										{currentParamConfig.min !== undefined && (
											<ReferenceLine
												...
												label={{
													position: "bottom",
													value: `MIN: ${currentParamConfig.min} ${currentParamConfig.unit}`,
													fill: "#f59e0b",
													fontSize: 10,
													fontWeight: "900",
													fontFamily: "monospace",
									...
									{isAiLoading ? (
										...
										<div className="flex items-center gap-2.5 text-neutral-400 h-full">
											...
											<p className="text-[10px] font-mono leading-relaxed">
												Klik tombol di sebelah kiri untuk menghasilkan ringkasan
												kausalitas kualitas air.
											</p>
										</div>
									)}
								</div>
						...
						<div className="p-5 border-b border-neutral-200 bg-neutral-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
							<div>
								<h3 className="text-sm font-bold uppercase flex items-center gap-2">
									<TestTubes className="w-4 h-4" /> Database Uji Laboratorium
								</h3>
								<p className="text-[10px] text-neutral-500 font-mono mt-0.5">
									Menampilkan {filteredData.length} data uji lab pada periode
									terpilih.
								</p>
							</div>
							...
							<div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
								...
								<div className="flex gap-2 w-full sm:w-auto">
									<button
										...
										className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 bg-green-50 text-green-700 text-[10px] font-bold border border-green-200 rounded hover:bg-green-100 transition-colors shadow-sm"
									>
										<FileSpreadsheet className="w-3 h-3" /> EXCEL
									</button>
									<button
										onClick={handleExportPDF}
										className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 bg-red-50 text-red-700 text-[10px] font-bold border border-red-200 rounded hover:bg-red-100 transition-colors shadow-sm"
									>
							...
							<table className="min-w-max w-full text-left border-collapse bg-white">
								<thead className="sticky top-0 z-20 shadow-sm">
									<tr className="bg-neutral-900 text-white text-[10px] uppercase tracking-wider font-bold">
										<th
											colSpan={3}
											className="px-4 py-2 border-r border-neutral-700 text-center"
										>
											INFORMASI UMUM
										</th>
										<th
											colSpan={4}
											className="px-4 py-2 border-r border-neutral-700 text-center bg-teal-900"
										>
											PENGUKURAN LAPANGAN
										</th>
										<th
											colSpan={9}
											className="px-4 py-2 border-r border-neutral-700 text-center bg-blue-900"
										>
											PENGUJIAN LABORATORIUM
										</th>
										<th
											colSpan={1}
											className="px-4 py-2 text-center bg-neutral-800"
										>
											KESIMPULAN
										</th>
									</tr>
									<tr className="bg-neutral-100 text-neutral-800 text-[10px] font-bold border-b border-neutral-300">
										<th
											rowSpan={2}
											className="px-3 py-2 border-r border-neutral-300 text-center"
										>
											NO
										</th>
										<th
											rowSpan={2}
											className="px-4 py-2 border-r border-neutral-300"
										>
											LOKASI UJI
										</th>
										<th
										...
										<th
											rowSpan={2}
											className="px-3 py-2 border-r border-neutral-300 text-center bg-teal-50"
										>
											pH
										</th>
										<th
										...
										>
											Suhu
											<br />
											<span className="text-[9px] font-mono text-teal-600 font-normal">
												°C
											</span>
										</th>
										<th
										...
										<th
											colSpan={2}
											className="px-3 py-2 border-r border-neutral-300 text-center bg-blue-50"
										>
											FISIKA
										</th>
										<th
											colSpan={5}
											className="px-3 py-2 border-r border-neutral-300 text-center bg-indigo-50"
										>
											ANORGANIK
										</th>
										<th
											colSpan={2}
											className="px-3 py-2 border-r border-neutral-300 text-center bg-purple-50"
										>
											MIKROBIOLOGI
										</th>
										<th rowSpan={2} className="px-4 py-2 text-center">
											STATUS
										</th>
									</tr>
									<tr className="bg-neutral-50 text-neutral-600 text-[9px] font-bold border-b border-neutral-300 shadow-sm">
										<th className="px-2 py-2 border-r border-neutral-300 text-center bg-blue-50/50">
											TDS
											<br />
											<span className="font-normal text-blue-500 font-mono">
												mg/l
											</span>
										</th>
										<th className="px-2 py-2 border-r border-neutral-300 text-center bg-blue-50/50">
										...
										<th className="px-2 py-2 border-r border-neutral-300 text-center bg-indigo-50/50">
											Fe
											<br />
											<span className="font-normal text-indigo-500 font-mono">
												mg/l
											</span>
										</th>
										<th className="px-2 py-2 border-r border-neutral-300 text-center bg-indigo-50/50">
										...
										<th className="px-2 py-2 border-r border-neutral-300 text-center bg-purple-50/50">
											Coliform
											<br />
											<span className="font-normal text-purple-500 font-mono">
												CFU/100ml
											</span>
										</th>
										<th className="px-2 py-2 border-r border-neutral-300 text-center bg-purple-50/50">
								...
								<tbody className="divide-y divide-neutral-200 text-xs text-neutral-800">
									{filteredData.length === 0 ? (
										<tr>
											...
											>
												...
												<p className="font-mono font-bold text-sm">
													DATA TIDAK DITEMUKAN / BELUM ADA DATA PADA PERIODE INI
												</p>
											</td>
										</tr>
									) : (
										filteredData.map((row, index) => (
											...
											>
												<td className="px-3 py-3 text-center border-r border-neutral-100">
													{index + 1}
												</td>
												<td className="px-4 py-3 font-bold text-neutral-900 border-r border-neutral-100 whitespace-nowrap">
													{row.lokasi}
												</td>
												<td className="px-2 py-3 text-center font-mono text-[10px] text-neutral-500 border-r border-neutral-100">
													{format(parseISO(row.tgl_uji), "dd/MM/yy")}
												</td>

												{/* PENGUKURAN LAPANGAN */}
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 bg-teal-50/10 ${isTidakMemenuhi("ph", row.ph) ? "text-red-600 font-bold" : ""}`}
												>
													{row.ph}
												</td>
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 bg-teal-50/10 ${isTidakMemenuhi("suhu", row.suhu) ? "text-red-600 font-bold" : ""}`}
												>
													{row.suhu}
												</td>
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 bg-teal-50/10 ${isTidakMemenuhi("klorin", row.klorin) ? "text-red-600 font-bold" : ""}`}
												>
													{row.klorin}
												</td>
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 bg-teal-50/10 ${isTidakMemenuhi("kekeruhan", row.kekeruhan) ? "text-red-600 font-bold" : ""}`}
												>
													{row.kekeruhan}
												</td>
												...
												{/* FISIKA */}
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 ${isTidakMemenuhi("tds", row.tds) ? "text-red-600 font-black" : "text-blue-700 font-bold"}`}
												>
													{row.tds}
												</td>
												{/* WARNA (Ditambahkan Modifier) */}
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 ${isTidakMemenuhi("warna", row.warna) ? "text-red-600 font-bold" : ""}`}
												>
													{row.warna_modifier === "<" ? "< " : ""}
													{row.warna}
												</td>
												...
												{/* ANORGANIK */}
												{/* FE (Ditambahkan Modifier) */}
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 ${isTidakMemenuhi("fe", row.fe) ? "text-red-600 font-bold" : ""}`}
												>
													{row.fe_modifier === "<" ? "< " : ""}
													{row.fe}
												</td>
												{/* MN (Ditambahkan Modifier) */}
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 ${isTidakMemenuhi("mn", row.mn) ? "text-red-600 font-bold" : ""}`}
												>
													{row.mn_modifier === "<" ? "< " : ""}
													{row.mn}
												</td>
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 ${isTidakMemenuhi("flourida", row.flourida) ? "text-red-600 font-bold" : ""}`}
												>
													{row.flourida}
												</td>
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 ${isTidakMemenuhi("nitrat", row.nitrat) ? "text-red-600 font-bold" : ""}`}
												>
													{row.nitrat}
												</td>
												<td
													className={`px-3 py-3 text-center font-mono border-r border-neutral-100 ${isTidakMemenuhi("nitrit", row.nitrit) ? "text-red-600 font-bold" : ""}`}
												>
													{row.nitrit}
												</td>
												...
												{/* COLIFORM */}
												<td
													className={`px-3 py-3 text-center font-mono font-bold border-r border-neutral-100 ${isTidakMemenuhi("coliform", row.coliform, row) ? "text-red-600 bg-red-50" : "text-neutral-700"}`}
												>
													{row.coliform_modifier === "<" ? "< " : ""}
													{row.coliform}
												</td>

												{/* ECOLI */}
												<td
													className={`px-3 py-3 text-center font-mono font-bold border-r border-neutral-100 ${isTidakMemenuhi("ecoli", row.ecoli, row) ? "text-red-600 bg-red-50" : "text-neutral-700"}`}
												>
													{row.ecoli_modifier === "<" ? "< " : ""}
													{row.ecoli}
												</td>
												...
												<td className="px-4 py-3 text-center bg-neutral-50/50">
													<span
														className={`px-2.5 py-1 rounded border text-[10px] font-bold uppercase tracking-wider ${getStatusBadge(row.status)}`}
													>
														{row.status}
													</span>
												</td>
								...
								{tableAverages && (
									<tfoot className="bg-neutral-900 text-white font-bold text-[11px] shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] relative z-20">
										<tr>
											<td
												colSpan={3}
												className="px-4 py-3.5 text-right border-r border-neutral-700 uppercase tracking-widest text-neutral-400"
											>
												Rata-Rata Keseluruhan
											</td>
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-teal-300">
												{tableAverages.ph}
											</td>
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-teal-300">
												{tableAverages.suhu}
											...
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-blue-300">
												{tableAverages.tds}
											</td>
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-blue-300">
												{tableAverages.warna}
											</td>
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-indigo-300">
												{tableAverages.fe}
											</td>
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-indigo-300">
												{tableAverages.mn}
											...
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-purple-300">
												{tableAverages.coliform}
											</td>
											<td className="px-3 py-3.5 text-center border-r border-neutral-700 font-mono text-purple-300">
												{tableAverages.ecoli}
							...
							<div className="bg-neutral-900 p-6 sm:p-8 text-white flex justify-between items-start relative overflow-hidden shrink-0">
								...
								<div className="relative z-10">
									<div className="flex items-center gap-3 mb-3">
										<span
											className={`px-2.5 py-1 rounded border text-[10px] font-bold uppercase tracking-wider ${getStatusBadge(selectedRow.status)}`}
										>
											{selectedRow.status}
										</span>
										<span className="text-[10px] font-mono text-neutral-400 bg-neutral-800 px-2 py-1 rounded-md border border-neutral-700">
											ID: {selectedRow.id}
										</span>
									</div>
									...
									<p className="text-sm text-neutral-400 font-mono mt-2 flex items-center gap-2">
										...
										{format(parseISO(selectedRow.tgl_uji), "dd MMMM yyyy", {
											locale: id,
										})}
									</p>
								</div>
								<button
									...
									className="p-2.5 bg-neutral-800 hover:bg-red-500 hover:text-white rounded-full transition-colors relative z-10 shadow-sm group"
								>
									<svg
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
									...
									<div className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm">
										...
										<div className="space-y-3.5">
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												<span className="text-xs text-neutral-500 font-bold">
													Suhu
												</span>
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("suhu", selectedRow.suhu) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.suhu}{" "}
											...
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("kekeruhan", selectedRow.kekeruhan) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.kekeruhan}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														NTU
													</span>
												</span>
											...
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("tds", selectedRow.tds) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.tds}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														mg/l
											...
											<div className="flex justify-between items-end">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("warna", selectedRow.warna) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.warna}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														TCU
									...
									<div className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm">
										...
										<div className="space-y-3.5">
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("ph", selectedRow.ph) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.ph}
												</span>
											</div>
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("klorin", selectedRow.klorin) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.klorin}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														mg/l
											...
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("fe", selectedRow.fe) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.fe}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														mg/l
											...
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("mn", selectedRow.mn) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.mn}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														mg/l
											...
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("flourida", selectedRow.flourida) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.flourida}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														mg/l
													</span>
												</span>
											...
											<div className="flex justify-between items-end border-b border-neutral-50 pb-1.5">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("nitrat", selectedRow.nitrat) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.nitrat}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														mg/l
											...
											<div className="flex justify-between items-end">
												...
												<span
													className={`font-mono font-black text-sm ${isTidakMemenuhi("nitrit", selectedRow.nitrit) ? "text-red-600" : "text-neutral-800"}`}
												>
													{selectedRow.nitrit}{" "}
													<span className="text-[10px] text-neutral-400 font-normal">
														mg/l
									...
									<div className="flex flex-col gap-6">
										<div className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm">
											...
											<div className="space-y-4">
												<div className="flex justify-between items-center bg-neutral-50 p-3 rounded-lg border border-neutral-100">
													...
													<span
														className={`font-mono font-black text-lg ${selectedRow.coliform > 0 ? "text-red-600" : "text-emerald-600"}`}
													>
														{selectedRow.coliform}{" "}
														<span className="text-[10px] text-neutral-400 font-normal">
															CFU
														</span>
													</span>
												...
												<div className="flex justify-between items-center bg-neutral-50 p-3 rounded-lg border border-neutral-100">
													...
													<span
														className={`font-mono font-black text-lg ${selectedRow.ecoli > 0 ? "text-red-600" : "text-emerald-600"}`}
													>
														{selectedRow.ecoli}{" "}
														<span className="text-[10px] text-neutral-400 font-normal">
															CFU
										...
										<div className="bg-neutral-900 p-5 rounded-xl border border-neutral-800 text-white shadow-inner flex-1 flex flex-col justify-center">
											<h3 className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2 flex items-center gap-2">
												<ShieldCheck className="w-3.5 h-.5" /> Laboratorium
												Penguji
											</h3>
