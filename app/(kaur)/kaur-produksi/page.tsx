...
export default function DataManajemenPage() {
	...
	const filteredData = dataLaporan.filter((row) => {
		const formattedDate = format(parseISO(row.tanggal), "MMMM yyyy", {
			locale: id,
		}).toLowerCase();
		const spamName =
		...
		return (
			formattedDate.includes(searchLower) || spamName.includes(searchLower)
		);
	});
		...
		if (modalMode === "tambah") {
			...
			if (error) {
				...
				showNotification(
					"success",
					"Data Tersimpan",
					"Data laporan produksi baru berhasil ditambahkan.",
				);
			}
			...
			if (error) {
				...
				showNotification(
					"success",
					"Data Diperbarui",
					"Pembaruan log operasional telah berhasil disimpan.",
				);
			}
		}
	...
	const requestDelete = (id: string) => {
		setAlertConfig({
			...
			message:
				"Apakah Anda yakin ingin menghapus log operasional ini? Tindakan ini bersifat permanen dan tidak dapat dibatalkan.",
			targetId: id,
		});
	};
			...
			setTimeout(
				() =>
					showNotification(
						"success",
						"Data Dihapus",
						"Log operasional berhasil dihapus dari sistem.",
					),
				300,
			);
		}
	...
	return (
		<div className="p-8 max-w-7xl mx-auto relative">
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
				<div>
					<h1 className="text-2xl font-black uppercase tracking-tight text-neutral-900 mb-1">
						Manajemen Data
					</h1>
				</div>
				<button
							...
							{loading ? (
								...
								<tr>
									<td
										...
										className="px-4 py-12 text-center text-neutral-400"
									>
										Belum ada data laporan yang sesuai.
									</td>
								</tr>
							) : (
								paginatedData.map((row) => (
									...
									>
										<td className="px-4 py-3 font-bold text-neutral-900 group-hover:text-blue-700 transition-colors">
											{format(parseISO(row.tanggal), "MMMM yyyy", {
												locale: id,
											}).toUpperCase()}
										</td>
										<td className="px-4 py-3 text-neutral-600">
											{row.spam_id === "cimahi_utara"
												? "SPAM Cimahi Utara"
												: "SPAM Pasirkaliki"}
										</td>
										...
										<td className="px-4 py-3">
											<div className="flex items-center justify-center gap-2">
												<button
													...
													}} // STOP PROPAGATION BIAR BARIS GAK KE KLIK
				...
				<div className="fixed inset-0 z-40 flex items-center justify-center bg-neutral-900/60 backdrop-blur-sm p-4">
					<div className="bg-white w-full max-w-2xl rounded-sm shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
						<div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 bg-neutral-50">
							...
							<button
								onClick={closeModal}
								className="p-1 hover:bg-neutral-200 rounded transition-colors text-neutral-500 hover:text-red-500"
							>
								<X className="w-5 h-5" />
							</button>
						</div>
						...
						{modalMode === "detail" ? (
							<div className="p-6 flex flex-col gap-6 bg-white">
								<div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 border-b border-neutral-100 pb-6">
									<div>
										<p className="text-[10px] font-bold uppercase text-neutral-400 font-mono mb-1">
											Bulan Laporan
										</p>
										<p className="text-base font-bold text-neutral-900">
											{formData.tanggal
												? format(parseISO(formData.tanggal), "MMMM yyyy", {
														locale: id,
													}).toUpperCase()
												: "-"}
										</p>
									...
									<div>
										...
										<p className="text-base font-bold text-neutral-900">
											{formData.spam_id === "cimahi_utara"
												? "SPAM Cimahi Utara"
												: "SPAM Pasirkaliki"}
										</p>
									</div>
									<div>
										<p className="text-[10px] font-bold uppercase text-teal-600 font-mono mb-1">
											Debit Air Baku
										</p>
										<p className="text-2xl font-mono font-bold text-teal-700">
											{formData.debit_air_baku}{" "}
											<span className="text-sm font-sans text-neutral-500 font-normal">
												LPS
											</span>
										</p>
									</div>
									<div>
										<p className="text-[10px] font-bold uppercase text-blue-600 font-mono mb-1">
											Volume Produksi Total
										</p>
										<p className="text-2xl font-mono font-bold text-blue-700">
											{Number(formData.volume_produksi).toLocaleString("id-ID")}{" "}
											<span className="text-sm font-sans text-neutral-500 font-normal">
												m³
											</span>
										</p>
									...
									<div>
										...
										<p className="text-lg font-mono font-bold text-neutral-800">
											{Number(formData.jam_operasi).toLocaleString("id-ID")}{" "}
											<span className="text-sm font-sans text-neutral-500 font-normal">
												Jam
											</span>
										</p>
								...
								<div className="grid grid-cols-2 gap-4 bg-amber-50/50 p-4 border border-amber-100 rounded-sm">
									<div>
										<p className="text-[10px] font-bold uppercase text-amber-600 font-mono mb-1">
											Pemakaian PAC
										</p>
										<p className="text-xl font-mono font-bold text-amber-700">
											{Number(formData.pemakaian_pac).toLocaleString("id-ID")}{" "}
											<span className="text-sm font-sans text-amber-600/60 font-normal">
												Kg
											</span>
										</p>
									</div>
									<div>
										<p className="text-[10px] font-bold uppercase text-neutral-500 font-mono mb-1">
											Pemakaian Kaporit
										</p>
										<p className="text-xl font-mono font-bold text-neutral-700">
											{Number(formData.pemakaian_kaporit).toLocaleString(
												"id-ID",
											)}{" "}
							...
							<form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
								<div className="flex gap-3 p-3 bg-blue-50 border border-blue-100 rounded-sm text-blue-800 text-xs font-mono">
									...
									<p>
										Pastikan data yang diinput adalah akumulasi/rekapitulasi
										total untuk bulan yang dipilih.
									</p>
								</div>
								<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
									<div className="space-y-1.5">
										...
										<input
											...
											className="w-full p-2.5 text-sm font-mono border border-neutral-300 rounded-sm focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all bg-white"
										/>
									</div>
									<div className="space-y-1.5">
										<label className="text-[10px] font-bold uppercase text-neutral-500 font-mono">
											Lokasi Instalasi (SPAM)
										</label>
										<select
											required
											value={formData.spam_id}
											onChange={(e) =>
												setFormData({ ...formData, spam_id: e.target.value })
											}
											className="w-full p-2.5 text-sm font-mono border border-neutral-300 rounded-sm focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all bg-white"
										>
									...
									<div className="space-y-1.5">
										...
										<input
											...
											value={formData.debit_air_baku}
											onChange={(e) =>
												setFormData({
													...formData,
													debit_air_baku: e.target.value,
												})
											}
											className="w-full p-2.5 text-sm font-mono border border-neutral-300 rounded-sm focus:border-blue-600 outline-none transition-all"
										/>
									</div>
									<div className="space-y-1.5">
										<label className="text-[10px] font-bold uppercase text-neutral-500 font-mono">
										...
										<input
											...
											value={formData.volume_produksi}
											onChange={(e) =>
												setFormData({
													...formData,
													volume_produksi: e.target.value,
												})
											}
											className="w-full p-2.5 text-sm font-mono border border-neutral-300 rounded-sm focus:border-blue-600 outline-none transition-all"
										/>
									...
									<div className="grid grid-cols-2 gap-3 space-y-0">
										<div className="space-y-1.5">
											<label className="text-[10px] font-bold uppercase text-amber-600 font-mono">
												Total PAC (Kg)
											</label>
											<input
												type="number"
												...
												required
												placeholder="0"
												value={formData.pemakaian_pac}
												onChange={(e) =>
													setFormData({
														...formData,
														pemakaian_pac: e.target.value,
													})
												}
												className="w-full p-2.5 text-sm font-mono border border-amber-200 bg-amber-50/30 rounded-sm focus:border-amber-500 outline-none transition-all"
											/>
										</div>
										<div className="space-y-1.5">
											<label className="text-[10px] font-bold uppercase text-neutral-500 font-mono">
											...
											<input
												...
												required
												placeholder="0"
												value={formData.pemakaian_kaporit}
												onChange={(e) =>
													setFormData({
														...formData,
														pemakaian_kaporit: e.target.value,
													})
												}
												className="w-full p-2.5 text-sm font-mono border border-neutral-300 bg-neutral-50 rounded-sm focus:border-neutral-500 outline-none transition-all"
											/>
										</div>
									</div>
								</div>
								<div className="flex justify-end gap-3 pt-6 border-t border-neutral-100 mt-2">
									<button
										type="button"
										onClick={closeModal}
										className="px-5 py-2.5 text-xs font-bold font-mono text-neutral-500 hover:text-neutral-900 hover:bg-neutral-100 rounded-sm transition-colors"
									>
										BATAL
									</button>
									<button
										...
										className="flex items-center gap-2 px-6 py-2.5 bg-neutral-900 hover:bg-blue-600 text-white text-xs font-bold font-mono rounded-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
									>
										{isSubmitting ? (
